...
-- -----------------------------------------------------
-- Table `JAVA19_shopping_diagram_to_sql`.`ORDER`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `JAVA19_shopping_diagram_to_sql`.`ORDER` ;

CREATE TABLE IF NOT EXISTS `JAVA19_shopping_diagram_to_sql`.`ORDER` (
  `ID` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `JAVA19_shopping_diagram_to_sql`.`ORDER_DETAIL` (
  ...
  UNIQUE INDEX `UNITE` (`ITEM_ID` ASC, `ORDER_ID` ASC) VISIBLE,
  CONSTRAINT `fk_ORDER_has_ITEM_ORDER1`
    FOREIGN KEY (`ORDER_ID`)
    REFERENCES `JAVA19_shopping_diagram_to_sql`.`ORDER` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
