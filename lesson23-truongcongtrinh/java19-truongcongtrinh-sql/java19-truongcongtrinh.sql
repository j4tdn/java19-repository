...
CREATE TABLE KHACHHANG(
	MaKH VARCHAR(50) PRIMARY KEY, -- thừa space
...
DELETE FROM CHITIET_SUDUNGDV;
DELETE FROM HOADON;
DELETE FROM DICHVU;
DELETE FROM MUCTIENGIO;
DELETE FROM PHONG;
...
SELECT PHONG.MaPhong, PHONG.MoTa, 
       SUM(TIMESTAMPDIFF(SECOND, HOADON.ThoiGianBatDauSD, HOADON.ThoiGianKetThucSD)) AS TongThoiGianSuDung
FROM HOADON
JOIN PHONG ON HOADON.MaPhong = PHONG.MaPhong
WHERE HOADON.ThoiGianBatDauSD >= '2014-02-01'
AND HOADON.ThoiGianBatDauSD <= '2015-02-28'
GROUP BY PHONG.MaPhong, PHONG.MoTa
...
WITH ServiceUsage AS (
    SELECT 
        MONTH(HOADON.ThoiGianBatDauSD) AS Thang,
        YEAR(HOADON.ThoiGianBatDauSD) AS Nam,
    ...
    FROM 
        CHITIET_SUDUNGDV
    JOIN 
        HOADON ON CHITIET_SUDUNGDV.MaHD = HOADON.MaHD
    JOIN 
        DICHVU ON CHITIET_SUDUNGDV.MaDV = DICHVU.MaDV
    WHERE 
        HOADON.ThoiGianBatDauSD >= '2014-01-01' 
        AND HOADON.ThoiGianBatDauSD < '2015-01-01'
    GROUP BY 
        MONTH(HOADON.ThoiGianBatDauSD), 
        YEAR(HOADON.ThoiGianBatDauSD),
        CHITIET_SUDUNGDV.MaDV, 
        DICHVU.TenDV
),
RankedServices AS (
    SELECT 
        ...
        TenDV, 
        TongSoLuong,
        ROW_NUMBER() OVER (PARTITION BY Thang, Nam ORDER BY TongSoLuong DESC) AS rnk
    FROM 
        ServiceUsage
)
SELECT 
